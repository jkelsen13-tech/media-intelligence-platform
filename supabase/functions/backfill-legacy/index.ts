...
async function topicsPass(supabase: any, cfg: any, report: any, deadline: number) {
  ...
  for (;;) {
    ...
    for (const a of data) {
      ...
      if (!nodeId) continue
      const added = await tagNodeTopics(supabase, nodeId, `${a.title}. ${a.summary ?? ''}`, floor)
      if (added > 0) {
        report.nodesTagged++
        report.topicRows += added
      }
    }
    if (data.length < 500) break
    from += 500
    if (Date.now() > deadline) {
      report.topicPassIncomplete = true
      break
    }
  }
}
...
Deno.serve(async (req) => {
  ...
  const mode = url.searchParams.get('mode') ?? (url.searchParams.get('reset') === '1' ? 'reset' : 'resume')
  ...
  try {
    if (mode === 'actors-topics') {
      // Additive, no reset: Step 6 actor graph + Step 8 topic tags for the
      // CURRENT arc layer. Safe to re-run; both passes are idempotent.
      await actorsPass(supabase, report, deadline)
      await topicsPass(supabase, cfg, report, deadline)
    ...
    const state = (await loadState(supabase)) ?? { phase: 'extract' as const }
    ...
    if (state.phase === 'extract') {
      while (Date.now() < deadline - 5_000) {
        ...
        if (!more) {
          await saveState(supabase, { phase: 'originate' })
          report.phaseTransition = 'extract->originate'
          break
        }
      }
      const { data: pending } = await supabase
        ...
        .select('id', { count: 'exact', head: true })
        .is('entities_extracted_at', null)
      report.extractRemaining = (pending as any)?.length ?? null
      return Response.json({ ok: true, done: false, ...report })
    }
    ...
    if (state.phase === 'originate') {
      ...
      if (report.originatePoolEmpty || report.originateExhausted) {
        await saveState(supabase, { phase: 'attach' })
        report.phaseTransition = 'originate->attach'
      }
      return Response.json({ ok: true, done: false, ...report })
    }
    ...
    if (state.phase === 'attach') {
      while (Date.now() < deadline - 5_000) {
        ...
        if (!more) {
          await saveState(supabase, { phase: 'done' })
          report.phaseTransition = 'attach->done'
          break
        }
      }
      const { count } = await supabase
        ...
        .is('arc_assign_attempted_at', null)
        .eq('is_digest', false)
      report.attachRemaining = count ?? null
      return Response.json({ ok: true, done: false, ...report })
    }

    return Response.json({ ok: true, done: true, ...report })
  } catch (err) {
    return Response.json({ ok: false, error: String(err), ...report }, { status: 500 })
  }
})
