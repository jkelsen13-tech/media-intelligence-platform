...
CI gate (**COMPLETE — verified 2026-07-28**): `.github/workflows/golden.yml` (push/PR/manual) + `test` job gating Pages deploy in `blank.yml`. Installed by the owner via the GitHub web editor (the G1 API token lacked the `workflow` scope); installed files byte-verified against the G1-prepared contents (golden.yml blob `2a7adc8f`, blank.yml blob `1238c6b9`). Verified successful runs: Golden regression suite run **30330593241** (run #1, success, head `9abdf556`) and Pages deploy run **30330593226** (run #103, success, gated by the `test` job; the gate also passed on deploy run 30330374637).
...
- **Suite alerts:** CI failure on push/PR → GitHub Actions notification to the
  repo owner; failed deploy gate blocks Pages deploy automatically (gate
  verified live 2026-07-28, runs 30330593241 / 30330593226).
