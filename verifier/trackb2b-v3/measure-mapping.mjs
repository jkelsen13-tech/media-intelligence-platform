// Verifier trackb2b-v3 (2026-08-18): re-confirms the Step 2b verification
// tests against the SHIPPED implementation AFTER the owner-ruled entity_type
// mapping correction (institution/other actors -> honest labels, UNGROUPED).
// T3.c is re-framed for the ungrouped-node regime (see README.md): a hull
// enclosing a DIFFERENT REGION's member is a false grouping claim (hard
// gate); a hull enclosing an UNGROUPED node is convex-hull geometry, recorded
// with labels, disambiguated by the node's own card. Read-only PostgREST.
// Usage: node verifier/trackb2b-v3/measure-mapping.mjs
...
// ---- T3: dense states against shipped seams ----
{
  ...
  // Split purity: foreign-REGION members inside a hull (false grouping —
  // hard gate) vs UNGROUPED nodes inside a hull (convex geometry over
  // no-membership nodes — recorded, disambiguated by the node's card label).
  let foreignRegionViolations = 0
  const ungroupedEnclosed = []
  for (const r of regions) {
    const memberIds = new Set(members.filter((m) => m.region === r.region).map((m) => m.id))
    for (const m of members) {
      if (memberIds.has(m.id) || !pointInHull(m, r.points)) continue
      if (m.region == null) ungroupedEnclosed.push(`${m.id} inside ${r.region}`)
      else foreignRegionViolations++
    }
  }
  check("T3.c' focused-view hulls enclose no FOREIGN-REGION node centers", foreignRegionViolations === 0,
    { regions: regions.map((r) => `${r.region}:${r.memberCount}`), foreignRegionViolations, ungroupedEnclosed })
